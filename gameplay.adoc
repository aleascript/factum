...
Nous indiquerons les ressources disponibles dans chacune des phases du jeu. Vous être libres ou pas de les utiliser ou d'en utiliser d'autres. Factum est pour cela très modulaire et facilement "customisable".
...
* *questions -> cadrage*: des questions pour cadrer l'univers et le type de jeu que l'on souhaite.
* *questions -> ambiance*: des questions si l'on veut spécifier l'ambiance.
...
* *lexiques -> génériques*: un certain nombre de lexiques génériques valables dans n'importe quel univers. (pour Cosmos et Chaos en mode tresse)
* *lexiques -> distances*: un petit lexique orienté oracle pour mesurer une distance (spatiale, temporelle, ....) (pour Cosmos et Chaos plutôt en mode tresse)
* *lexiques -> directions symboliques*: un petit lexique orienté oracle pour répondre à des questions ouvertes (pour Cosmos plutôt en mode tresse)
